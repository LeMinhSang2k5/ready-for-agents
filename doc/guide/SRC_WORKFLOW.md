...
7. [Đọc / ghi file: `fs/](#7-đọc--ghi-file-fs)`
8. [Detectors: `detectors/](#8-detectors-detectors)`
9. [Generators: `generators/](#9-generators-generators)`
10. [Public API: `index.ts](#10-public-api-indexts)`
...
flowchart TD
  ...
  G --> H[in Checks + Score]
  H --> I{hasCriticalFailure?}
  I -->|có fail| J[exit 1]
  I -->|không| K[exit 0]
```

**Output:** chỉ in terminal — **không ghi file**. Dùng trước/sau `init` để biết project còn thiếu gì.
...
- Đăng ký lệnh `init` và `doctor` với **commander**
- Truyền options vào `runInit()` / `runDoctor()`
...
Không có `--dry-run` / `--force` — `doctor` không ghi file.
...
| 3 | In từng dòng `formatCheckLine` | ✓ pass / ! warn / ✗ fail (+ `detail` dim) |
| 4 | `formatScore(result)` | `Score: 6/11 · 4 warnings · 0 failures` |
| 5 | `hasCriticalFailure(result) ? 1 : 0` | Có bất kỳ check `fail` → exit 1 |
...
5.–7. `AGENTS.md`, `PROJECT_CONTEXT.md`, `COMMANDS.md` — **warn** nếu thiếu  
8.–10. `dev` / `build` / `test` script — **warn** nếu thiếu  
...
- `runInit`, `runDoctor`, `readProject`, `generateAllFiles`
...
Chạy: `pnpm test` (58 tests trong `tests/`, gồm `doctor.test.ts`, `generators.test.ts`).
...
*Tài liệu sinh cho codebase `agent-context-kit` v0.1.x — đã có `init` + `doctor`; cập nhật khi thêm `update` hoặc hỗ trợ ngôn ngữ khác.*
