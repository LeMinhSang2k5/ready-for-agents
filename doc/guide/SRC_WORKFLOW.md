...
- Đăng ký lệnh `init`, `update`, `diff`, `ci`, `doctor`, `prompt`, `index`, `config init` với **commander**
- Truyền options vào `runInit()` / `runUpdate()` / `runDiff()` / `runCi()` / `runDoctor()` / `runPrompt()` / `runIndex()` / `runConfigInit()`
...
| `--copilot`    | Với `--fix`, include Copilot instructions        |
...
Lọc trong `OUTPUT_FILES` (core files, optional agent files, workflow CI) — file nào `existsSync` thì có trong danh sách.
...
- **Notes:** README thiếu, PM fallback, hoặc “Generated by rfa init”
...
Chạy: `pnpm test` (279 tests trong `tests/`, gồm `doctor.test.ts`, `generators.test.ts`, `init-safety.test.ts`, `update.test.ts`, `ci-diff.test.ts`, `config-index.test.ts`, `cli.test.ts`, `prompt.test.ts`, `prompt-examples.test.ts`, `prompt-quality.test.ts`).
